...
- `Physics::FindBestHook`は選択距離、選択角度、遮蔽を検証して最良Hookを返す。
...
- `WireRenderer`は通常色、高張力色、破断色、ワールド半径、発光、たるみ、分割数を持つ。壁による遮蔽を反映し、距離に応じた太さで縄状に表示する。
- `Renderer::SetColor`と`SetEmission`により、質量を正規化した連続色やHookの視認性をScriptから変更できる。
- `GameObject::Create`、`SetParent`、`GetParent`、`GetChild`で実行時Hierarchyを構築できる。
...
desc.maximumLength = Distance(firstAnchor, secondAnchor); // 接続距離の制限ではなく初期ロープ長
...
| 項目 | 値 |
| --- | --- |
| `kEditorScriptApiVersion` | **7** |
| `EditorScriptRuntimeApi` のEntry数 | **235** |
| 高水準Wrapper Class数 | **61**（`EditorNativeScriptRuntime` と基底 `EditorNativeScript` を含む） |
| C++ Script Template数 | **26** |
| Component種類数 | **280**（うち追加可能276） |

以前この文書が基準にしていた「Runtime API Entry 218件」は、Audio 6件・`GetHealth`・Rail速度プロファイル3件・階層除外Raycast・Effekseer 3件・`PlayVfxAtPosition` を含む11件の追加前の値である。

### ABI互換の原則

`EditorScriptRuntimeApi` は**必ず構造体末尾へ追記する**。既存Entryの順序・型・引数は変更しない。このため、古いDLLを新しいEditorで動かしても、そのDLLが知っているEntryの位置はずれない。

Script側は必ず次の2段階で確認してから呼ぶ。この2つを飛ばすと、Editorのバージョンが古いときにnull呼び出しでクラッシュする。
...
| 7 | `EditorScriptVector2` | `GetMousePosition` | `（なし）` | なし（低水準のみ） |
...
| 入力Action | `GetActionVector2` / `IsActionPressed` / `WasActionJustPressed` / `GetMousePosition` | 前3つはOwnerに `PlayerInput` が必要。ActionMap名とAction名は Input Actions Asset の綴りと完全一致させる。`GetMousePosition` は引数なしで、Game View基準の位置を返す。 |
...
Inspector表示名「カメラ」／カテゴリ「カメラ」／公開Field 11件。
...
今回の汎用Componentアクセスを含めると、Scriptから利用できる公開面は「Runtime APIが235個」だけではない。役割の違う次の5層で構成される。

| 層 | 現行件数 | 意味 |
| --- | --- | --- |
| Runtime API Entry | 235 | DLLとEditorを接続する低水準関数ポインタ。 |
...
| 照合対象 | 結果 |
| --- | --- |
| `EditorScriptRuntimeApi` 宣言名と全235件索引 | 235 / 235一致、重複なし。 |
...
ただし、この一致は「宣言・型・名前・公開Fieldを文書から漏らしていない」という意味である。文字列、Asset参照、配列、複合要素、未登録Vector2、Subsystem再構築、Component固有の実行操作まで汎用Fieldアクセスだけで完結するという意味ではない。それらは本章前半の専用Wrapper、低水準Entry、またはComponent個別のRuntime契約を使う。
