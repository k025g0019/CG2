...
namespace EditorSharedState {
	...
	// GPU UploadBuffer と OBJ 読み込みに使う helper 宣言。
	...
	inline D3D12_CPU_DESCRIPTOR_HANDLE GetCPUDescriptorHandle(
		...
		// handle は Heap 先頭の CPU DescriptorHandle。
	...
	inline D3D12_GPU_DESCRIPTOR_HANDLE GetGPUDescriptorHandle(
		...
		// handle は Heap 先頭の GPU DescriptorHandle。
	...
	inline DirectX::ScratchImage LoadTexture(const std::wstring& filePath) {
		...
		// LoadFromWICFile は png などの画像を sRGB 前提で読み込む。
		assert(SUCCEEDED(hr));
	...
	inline ID3D12Resource* UploadTextureData(
		...
		// intermediateSize は全 mip を GPU Texture へコピーするために必要な UploadBuffer サイズ。
		ID3D12Resource* intermediateResource = CreateBufferResource(device, intermediateSize);
		// intermediateResource は texture へコピーするための一時 UploadBuffer。
	...
	inline MaterialData LoadMaterialTemplateFile(const std::string& directoryPath, const std::string& filename) {
		...
		while (std::getline(file, line)) {
			...
			if (identifier == "map_Kd") {
				...
				// OBJ から見た相対パスを、読み込みに使える directoryPath 付きパスへ直す。
			}
		}
		...
		while (std::getline(file, line)) {
			...
			if (identifier == "v") {
				...
				for (int32_t faceVertex = 0; faceVertex < 3; ++faceVertex) {
					...
					// vertexStream は '/' 区切りの index 文字列を分解するための stream。
					...
					// OBJ の index は 1 始まりなので、C++ 配列用に 0 始まりへ直す。
				...
				// .mtl を読み込み、Texture パスを modelData に保持する。
			}
		}
...
namespace EditorSharedState {
	...
	constexpr uint32_t kRuntimeShadowMapSize = 2048;  // 影用 DepthTexture の解像度。
	constexpr uint32_t kRuntimeShadowSrvDescriptorIndex = 15;  // 0 は ImGui、1-4 は固定 Texture、16 以降は個別 Texture。
	...
	// g_*DescriptorHeap は RTV / SRV / DSV をまとめて保持する DescriptorHeap。
	...
	inline ID3D12Resource* g_shadowMapResource = nullptr;  // 平行光源から見た深度を書き込む影用 DepthTexture。
	...
	// g_rootSignature / g_graphicsPipelineState は Draw 時に使う固定 GPU Pipeline 設定。
	...
	// g_editorWindowWidth / Height は ImGui と DirectX viewport が共有する Window サイズ。
	...
	// g_editorSceneObjectManager は GameObject と対応する DirectX 描画用 SceneObject を持つ。
	inline int32_t g_selectedPlacedSceneObjectIndex = -1;
	// g_selectedPlacedSceneObjectIndex は選択中 SceneObject の配列 index。-1 は未選択。
	...
	// g_textureFilePaths は GPU に読み込む標準 Texture の UTF-16 パス。
	inline std::string g_textureFilePathStrings[kRuntimeTextureCount];
	// g_textureFilePathStrings / g_editorTextureFilePaths は Project パネルで扱う UTF-8 パス。
	...
	inline auto g_sceneRangeStart = ImVec2(0.0f, 0.0f); // g_sceneRangeStart / End は SceneView 矩形選択の開始点と現在点。
	...
	// g_isEditorSceneInitialized / RuntimeInitialized は Scene と Play Runtime の初期化済みフラグ。
	inline bool g_isEditorRuntimeInitialized = false;

	inline int32_t g_selectedEditorGameObjectId = -1;
	// g_selectedEditorGameObjectId は Inspector / Hierarchy で選択中の GameObject ID。
	inline int32_t g_previousSelectedEditorGameObjectId = -1;
	// g_previousSelectedEditorGameObjectId は選択変更検出用の前回 GameObject ID。
	...
	// g_editorSelectionManager は GameObject 選択と SceneObject 選択を同期する。
	inline EditorSceneSynchronizer g_editorSceneSynchronizer;
	// g_editorSceneSynchronizer は EditorScene から描画用 SceneObject を作る。
	...
	inline void WaitForGpu() {
		...
		// Signal は CommandQueue へ、ここまでの GPU 作業番号を登録する。
	...
	inline void UpdateEditorLayout() {
		...
		// 各パネル幅・高さを操作可能な範囲に制限し、SceneView が潰れないようにする。
		...
		// SceneView 幅は Window 幅から左・右パネルを引いた残り。
		g_editorSceneHeight = g_editorWindowHeight - g_editorSceneY - g_editorBottomHeight;
		// SceneView 高さは Window 高さから上部領域と下部パネルを引いた残り。
	...
	inline void ResizeRenderTargets(uint32_t width, uint32_t height) {
		...
		for (uint32_t bufferIndex = 0; bufferIndex < kRuntimeSwapChainBufferCount; bufferIndex++) {
			...
			// Resize 後の back buffer を取得して RTV を再作成する。
			assert(SUCCEEDED(resizeResult));
		...
		// DepthStencil も新しい描画サイズに合わせて再作成する。
