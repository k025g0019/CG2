...
struct EditorOceanRenderSettings {
	...
	int32_t debugView = 0;  // Ocean Debug View 番号。0=通常描画
	float perPixelDisplacementStrength = 0.0f;  // 近景だけへ追加する画素単位のMedium Wave奥行き
	int32_t perPixelDisplacementSteps = 4;  // 固定反復にしてGPU負荷の上限を明確にする
	float perPixelDisplacementDistance = 45.0f;  // 遠景ではGeometry波だけへ戻す距離
	bool gpuTessellationEnabled = false;  // trueなら画面上の三角形サイズからGPUで分割係数を決める
	float tessellationTargetPixels = 12.0f;  // 細分化後の辺の目標Pixel長
	float tessellationMaximumFactor = 4.0f;  // 過剰細分化を防ぐ最大係数
