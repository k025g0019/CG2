// このファイルは Engine が自動生成します。ユーザーコードは ennsui_fbxScript.cpp へ記述してください。
...
namespace {
	...
	struct ScriptInstance {
		...
		ennsui_fbxScript script;
	};

	ennsui_fbxScript& GetMetadataState() {
		static ennsui_fbxScript metadataState;
...
extern "C" __declspec(dllexport) void EditorScript_UpdateInstance(void* instance, float deltaTime) {
	ScriptInstance* scriptInstance = GetScriptInstance(instance);
	if (scriptInstance != nullptr) {
		static_cast<EditorNativeScript&>(scriptInstance->script).Update(scriptInstance->gameObjectId, deltaTime);
	}
}
