...
namespace {
	...
	std::unordered_map<int32_t, en_fbxScript> scriptStates;  // GameObject ごとの実行状態を保持する。

	en_fbxScript& GetState(int32_t gameObjectId) {
...
extern "C" __declspec(dllexport) void EditorScript_Start(int32_t gameObjectId) {
	...
	en_fbxScript& state = GetState(gameObjectId);  // この GameObject 専用の状態を取り出す。
...
extern "C" __declspec(dllexport) void EditorScript_Update(int32_t gameObjectId, float deltaTime) {
	...
	en_fbxScript& state = GetState(gameObjectId);  // Update 内で使う速度などを持つ状態。
	...
	force.y = -100.0f;  // 上方向に力を加える例。Rigidbody があれば上方向に移動する。
