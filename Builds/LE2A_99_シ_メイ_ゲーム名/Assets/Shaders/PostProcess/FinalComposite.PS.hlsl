...
struct FinalCompositeConstants
{
    ...
    // Final Composite診断表示。0=通常。1-4はsceneColorを50%グレーへ強制し、
    // LocalContrast/Bloomを個別に有効化して、どちらが元画像の模様(泡・Sun反射等)を
    // 再注入しているか切り分ける(1=基準のみ 2=LocalContrastのみ 3=Bloomのみ 4=両方)。
    // Inspectorの「診断表示」から設定する(PostProcessコンポーネントcompositeDebugView)。
    float debugView;
    float3 bloomTint;
    // Ocean Debug View(0以外)がシーン内で有効なとき1.0。手動のdebugViewとは独立して、
    // ApplyLocalContrast/BloomがOceanのDebug単色出力へ元の海面模様を再注入するのを防ぐ。
    float oceanDebugActive;
...
float4 main(PixelShaderInput input) : SV_TARGET0
{
    ...
    float3 sceneColor = float3(redChannel, greenChannel, blueChannel);
    ...
    // Final Composite診断表示。0=通常(以下は一切通常描画へ影響しない)。1-4は原因切り分け用に
    // sceneColorを50%グレーへ強制し、LocalContrast/Bloomを個別にON/OFFする。
    //   1=A 基準のみ(両方OFF)   2=B LocalContrastのみ   3=C Bloomのみ   4=D 両方
    // LocalContrastはApplyLocalContrast内部でgSceneColorを直接再サンプルするため、
    // sceneColorを固定してもLocalContrastが有効なら元画像の模様が再注入され得る。
    const int compositeDebugView = int(gFinalComposite.debugView + 0.5f);

    if (compositeDebugView != 0)
    {
        sceneColor = float3(0.5f, 0.5f, 0.5f);
    }

    // Ocean自身のDebug View(1-46)が有効なシーンでは、compositeDebugView(手動診断)を
    // 切り替えていなくてもLocalContrast/Bloomを自動的に無効化する。sceneColorはここでは
    // 上書きしない(Oceanの単色/差分Debug出力をそのまま最終出力へ通す)。
    const bool isOceanDebugActive = gFinalComposite.oceanDebugActive >= 0.5f;

    const bool localContrastActive =
        !isOceanDebugActive &&
        (compositeDebugView == 0 || compositeDebugView == 2 || compositeDebugView == 4);
    const bool bloomActive =
        !isOceanDebugActive &&
        (compositeDebugView == 0 || compositeDebugView == 3 || compositeDebugView == 4);

    float3 color = localContrastActive
        ? ApplyLocalContrast(sceneColor, input.texcoord, gFinalComposite.localContrast)
        : sceneColor;
    ...
    // 番号の対応は EditorScene.h の compositeToneMappingMode と
    // Inspector の "トーンマップ" コンボの並びに合わせる。
    //   0=Reinhard / 1=Filmic / 2=Timothy / 3=Uncharted2 / 4=ACES
    // (以前ここが1つずれており、Inspectorで"ACES"を選ぶとUncharted2が走っていた。)
    if (gFinalComposite.toneMappingMode < 0.5f)
    {
        color = ReinhardToneMapExt(color, whitePoint);
    }
    else if (gFinalComposite.toneMappingMode < 1.5f)
    {
        color = FilmicToneMap(color / whitePoint);
    }
    else if (gFinalComposite.toneMappingMode < 2.5f)
    {
        color = TimothyToneMap(color / whitePoint);
    }
    else if (gFinalComposite.toneMappingMode < 3.5f)
    {
