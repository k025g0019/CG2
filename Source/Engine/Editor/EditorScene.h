...
enum class EditorComponentType {
	...
	SceneStreaming,
	// Text/TextMeshProUGUIへ汎用的な演出(フェード・タイプライター・拡縮ポップ・点滅)を適用する
	TextEffect,
...
// Ocean Debug View の表示モード数。0=通常描画、1..14 が各成分の単体表示。
// OceanSurface.PS.hlsl 冒頭のコメント一覧および Inspector の項目数と一致させること。
...
struct EditorComponent {
	...
	// FinalComposite診断表示。0=通常。1-4はsceneColorを50%グレーへ強制し、
	...
	int32_t oceanDebugView;  // Ocean Debug View 番号。0=通常描画、1以上で各成分を単体表示
	float oceanPerPixelDisplacementStrength;  // 近景のMedium Waveを画素単位で奥行き補正する強さ
	int32_t oceanPerPixelDisplacementSteps;  // 画素変位の固定レイ反復回数
	float oceanPerPixelDisplacementDistance;  // 画素変位を適用するカメラ距離
	bool oceanGpuTessellationEnabled;  // trueなら画面投影サイズに応じてOcean三角形をGPU細分化する
	float oceanTessellationTargetPixels;  // 細分化後の辺が目標とする画面上のPixel長
	float oceanTessellationMaximumFactor;  // Hardware Tessellationの最大分割係数
	// Buoyancy 設定
...
std::string waveStartedActionName;  // 条件成立時に通知する任意Script Action
	...
	bool surfaceWakeAffectOceanSurface;
	float surfaceWakeWaveAmplitudeScale;
	float surfaceWakeWaveRadiusScale;
	...
	// TextEffect 設定とRuntime値
	// 出現演出(1回だけ再生)と常時演出(表示され続ける間ずっと動く)は独立して同時にかけられる。
	int32_t textAppearEffectType;  // 0=None,1=フェードイン,2=タイプライター,3=スケールポップ
	float textAppearDuration;
	float textAppearDelay;
	float textAppearParamA;  // Typewriter=1秒あたりの文字数, ScalePop=オーバーシュート倍率
	int32_t textContinuousEffectType;  // 0=None,1=点滅,2=レインボー,3=波,4=発光,5=輪郭の発光,6=色収差,7=微振動,8=不規則点滅,9=脈動,10=残像,11=簡易グリッチ
	float textContinuousDelay;
	float textContinuousParamA;
	float textContinuousParamB;
	float textContinuousParamC;
	...
	float sceneTransitionOutDuration;  // 現在Sceneを覆うまでの時間
	float sceneTransitionHoldSeconds;  // 覆った状態で静止する時間
	float sceneTransitionInDuration;  // 新Sceneを見せる時間
	Vector3 sceneTransitionColor;
	int32_t sceneTransitionCameraDiveSourceGameObjectId;  // -1なら遷移開始時の現在Camera位置を基準にする
	Vector3 sceneTransitionCameraDivePositionOffset;  // 落下開始位置(基準WorldPositionからの相対Offset)
