...
void py_init_module_hello_imgui(nb::module_& m)
{
    ...
    // Internal accessor used by the Python async runners (immapp.run_async,
    // hello_imgui.run_async, immapp.nb.start) to pace their render loop.
    // Not part of the public API.
    m.def("_priv_idle_frame_wait_duration_for_python_async_io",
          &HelloImGui::Internal::_PrivIdleFrameWaitDurationForPythonAsyncIo);
    ...
    auto pyClassAppWindowParams =
        ...
        .def_rw("emscripten_allow_browser_zoom_shortcuts", &HelloImGui::AppWindowParams::emscriptenAllowBrowserZoomShortcuts, " `emscriptenAllowBrowserZoomShortcuts`: _bool, default=true_. (For Emscripten with GLFW backend only)\n If True, browser zoom shortcuts (Ctrl/Cmd + Plus/Minus/0) are forwarded to the browser\n instead of being captured by the application.\n Set to False if your application needs to handle these key combinations itself.")
    ...
    auto pyClassRunnerCallbacks =
        ...
        .def("__init__", [](HelloImGui::RunnerCallbacks * self, const std::optional<const VoidFunction> & ShowGui = std::nullopt, const std::optional<const VoidFunction> & ShowMenus = std::nullopt, const std::optional<const VoidFunction> & ShowAppMenuItems = std::nullopt, const std::optional<const VoidFunction> & ShowStatus = std::nullopt, const std::optional<const VoidFunction> & PostInit_AddPlatformBackendCallbacks = std::nullopt, const std::optional<const VoidFunction> & PostInit = std::nullopt, const std::optional<const VoidFunction> & LoadAdditionalFonts = std::nullopt, HelloImGui::DefaultIconFont defaultIconFont = HelloImGui::DefaultIconFont::FontAwesome4, const std::optional<const VoidFunction> & SetupImGuiConfig = std::nullopt, const std::optional<const VoidFunction> & SetupImGuiStyle = std::nullopt, const std::optional<const VoidFunction> & RegisterTests = std::nullopt, bool registerTestsCalled = false, const std::optional<const VoidFunction> & BeforeExit = std::nullopt, const std::optional<const VoidFunction> & BeforeExit_PostCleanup = std::nullopt, const std::optional<const VoidFunction> & PreNewFrame = std::nullopt, const std::optional<const VoidFunction> & PostNewFrame = std::nullopt, const std::optional<const VoidFunction> & BeforeImGuiRender = std::nullopt, const std::optional<const VoidFunction> & AfterSwap = std::nullopt, const std::optional<const VoidFunction> & CustomBackground = std::nullopt, const std::optional<const VoidFunction> & PostRenderDockableWindows = std::nullopt, const std::optional<const VoidFunction> & ThemeChanged = std::nullopt, const std::optional<const AnyEventCallback> & AnyBackendEventCallback = std::nullopt)
        {
            ...
            if (PostNewFrame.has_value())
                r_ctor_->PostNewFrame = PostNewFrame.value();
            else
                r_ctor_->PostNewFrame = HelloImGui::EmptyVoidFunction();
            ...
            if (ThemeChanged.has_value())
                r_ctor_->ThemeChanged = ThemeChanged.value();
            else
                r_ctor_->ThemeChanged = HelloImGui::EmptyVoidFunction();
        ...
        .def_rw("post_new_frame", &HelloImGui::RunnerCallbacks::PostNewFrame, " `PostNewFrame`: You can here add a function that will be called at each frame,\n  just after the call to ImGui::NewFrame(), and before any Gui code.")
    ...
    auto pyClassRunnerParams =
        ...
        .def("__init__", [](HelloImGui::RunnerParams * self, const std::optional<const HelloImGui::RunnerCallbacks> & callbacks = std::nullopt, const std::optional<const HelloImGui::AppWindowParams> & appWindowParams = std::nullopt, const std::optional<const HelloImGui::ImGuiWindowParams> & imGuiWindowParams = std::nullopt, const std::optional<const HelloImGui::DockingParams> & dockingParams = std::nullopt, const std::optional<const std::vector<HelloImGui::DockingParams>> & alternativeDockingLayouts = std::nullopt, bool rememberSelectedAlternativeLayout = true, const std::optional<const HelloImGui::BackendPointers> & backendPointers = std::nullopt, const std::optional<const HelloImGui::RendererBackendOptions> & rendererBackendOptions = std::nullopt, HelloImGui::PlatformBackendType platformBackendType = HelloImGui::PlatformBackendType::FirstAvailable, HelloImGui::RendererBackendType rendererBackendType = HelloImGui::RendererBackendType::FirstAvailable, HelloImGui::IniFolderType iniFolderType = HelloImGui::IniFolderType::CurrentFolder, std::string iniFilename = "", bool iniFilename_useAppWindowTitle = true, bool iniDisable = false, bool iniClearPreviousSettings = false, bool appShallExit = false, const std::optional<const HelloImGui::FpsIdling> & fpsIdling = std::nullopt, const std::optional<const HelloImGui::DpiAwareParams> & dpiAwareParams = std::nullopt, bool useImGuiTestEngine = false, int emscripten_fps = 0)
        {
            ...
            r_ctor_->iniClearPreviousSettings = iniClearPreviousSettings;
        ...
        .def_rw("ini_disable", &HelloImGui::RunnerParams::iniDisable, " `iniDisable`: _bool, default = false_.\n If True, do not save or load any settings to or from an ini file.")
        .def_rw("ini_clear_previous_settings", &HelloImGui::RunnerParams::iniClearPreviousSettings, " `iniClearPreviousSettings`: _bool, default = false_.\n If True, delete any previous settings ini file at application startup.")
    ...
    m.def("image_and_size_from_encoded_data",
